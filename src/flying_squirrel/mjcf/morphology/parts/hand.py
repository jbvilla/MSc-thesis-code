...
class MJCFFlyingSquirrelHand(MJCFMorphologyPart):
    def __init__(
            self,
        ...
        """
        Initialize the super class of the MJCFFlyingSquirrelHand
    ...
    def _build(self, in_plane_joint_axis, out_of_plane_joint_axis, *args, **kwargs) -> None:
        ...
        self._hand_specification = self.morphology_specification.forelimb_specification.hand_specification
        self._build_box()
        self._build_wing_tip()
        self._build_connector()
        self._configure_joints()
        self._configure_actuators()

    def _build_box(self) -> None:
        length = self._hand_specification.length.value
        width = self._hand_specification.width.value
        height = self._hand_specification.height.value
    ...
    def _build_wing_tip(self) -> None:
        length = self._hand_specification.wing_tip_length.value
        radius = self._hand_specification.wing_tip_radius.value

        self._wing_tip_body_name = f"{self.base_name}_wing_tip_body"

        self._wing_tip_body = self.mjcf_body.add(
            "body",
            name=self._wing_tip_body_name,
            pos=[0, 0, 0],
            euler=[0, 0, np.deg2rad(145)],
        )

        self._wing_tip = self._wing_tip_body.add(
            "geom",
            name=f"{self.base_name}_wing_tip",
            type="capsule",
    ...
    def _build_connector(self) -> None:
        radius = self._hand_specification.radius_connector.value
    ...
    def _configure_joints(self) -> None:
        self._in_plane_joint = self._configure_joint(
            ...
            joint_specification=self._hand_specification.in_plane_joint_specification,
        )
        ...
        self._out_of_plane_joint = self._configure_joint(
            ...
            joint_specification=self._hand_specification.out_of_plane_joint_specification,
        )
